class Product
  attr_reader :id, :name, :provinces, :months, :context

  def initialize(id:, name:, provinces:, context:, months:)
    @id = id
    @name = name
    @provinces = provinces
    @months = months
  ...
  def self.all
    ...
    @@list.each_with_index do |item, index|
      products << new(id: index, name: item[:name], provinces: item[:provinces], context: item[:context], months: item[:months])
    end
    products
  end
